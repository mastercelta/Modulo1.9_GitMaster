...
(function ($) {
    ...
    $.fn.extend({
    	createViewProcess: function(options_user){
    		try{
    		  var parameters = cenposGlobalParam = $.extend($.fn.createViewProcess.options, options_user );
						
              if($.fn.isNullorEmpty(parameters.url)) throw "The Url Parameter is empty";
		 
			  var elementObject = this;
			  $.fn.createiframeViewProcess(elementObject, parameters);
		...
		sendAction: function(type){
			if($(".framecenpos").length){
				windowsProxyCenpos.post(type)
			}
		},
    });

	$.fn.createViewProcess.options = {
		url: "",
		verifyingpost: "",
		type : "Void",
		flush: false,
    ...
    $.fn.error = function(exception) {
	    if (!$.fn.isNullorEmpty(exception.message)) {
	        alert("Excepction: " + exception.message);
	    } else if(!$.fn.isNullorEmpty(exception.description)){
			alert("Excepction: " + exception.description);
	    }else{
	    	alert(exception);
	    }
	}
  ...
  $.fn.isDefined = function (variable) {
	...
	$.fn.createiframeViewProcess = function (element, parameters) {
		var urlfinal = parameters.url;
		var res = urlfinal.substr(urlfinal.length - 1);
		urlfinal = res == "/" ? urlfinal : urlfinal + "/";
   		urlfinal = parameters.url + parameters.type+"/";
		if(parameters.flush) urlfinal +="?flush=true";
		var nameForm = "CenposForm" + Math.floor((Math.random() * 99999) + 90000);
		var nameIframe = "CenposIFrame" + Math.floor((Math.random() * 99999) + 90000);
		var submitIframe = "CenposSubmitForm" + Math.floor((Math.random() * 99999) + 90000);
		
		$(element).append("<form id='" + nameForm + "' action='" + urlfinal + "' target='" + nameIframe + "' method='POST'>");
		
		$("#"+nameForm).append("<input  type='hidden' value='"+parameters.verifyingpost+"' name='verifyingpost' />"); 	
		$("#"+nameForm).append("<input  type='hidden' value='"+JSON.stringify(parameters.data)+"' name='dataconfig' />");

		if(!$.fn.isNullorEmpty(parameters.params) && parameters.params.lenght > 0){
			parameters.params.forEach(function(ev,ind) {
				$("#"+nameForm).append("<input  type='hidden' value='"+ev+"' name='"+ind+"' />");
			});
		}	

		$("#"+nameForm).append("<input id='"+submitIframe+"' type='submit' style='opacity:0; height:0; width:0' />"); 		
		
		$(element).append("<iframe frameBorder='0' id='cenposPayIFrameId' name='" + nameIframe + "' class='framecenpos'  src='' width='" + parameters.width + "' height='" + parameters.height + "'   ></iframe>");
		
		$("#"+submitIframe).trigger("click");
		
		if ($.fn.isDefined(parameters.Open)) parameters.Open();
		windowsProxyCenpos = new Porthole.WindowProxy(parameters.url + 'proxy.html', nameIframe);
		windowsProxyCenpos.addEventListener($.fn.onMessageViewProcess);
   }
   ...
   $.fn.getUrlVars =  function () {
    ...
    $.fn.onMessageViewProcess = function (messageEvent) {
		if ($.fn.isDefined(cenposGlobalParam.callback)) {
			cenposGlobalParam.callback(messageEvent.data);
		}
	}
})(jQuery);